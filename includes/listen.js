...
(async function () {
	...
	logger(`${api.getCurrentUserID()} - [ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "This bot was made by Ayaan and SpermLord FIXING FILE CHORU TIKTOKERS" : global.config.BOTNAME}`, "[ BOT INFO ]");
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`✦Ye Bot Is Group Main Approved Nahi Hai So Approved Ke LiYe Request Kre_Ase: ${prefix}request\n☆FaceBook Owner Link\n✦ŶÁМĔĔŃ ĶĤÁŃ ✦\nhttps://m.facebook.com/yameen99king\n✦𝐁𝐎𝐓 𝐔𝐍𝐃𝐄𝐑 𝐏𝐑𝐎𝐓𝐄𝐂𝐓𝐄𝐃 𝐁𝐘 𝐎𝐖𝐍𝐄𝐑✦`, event.threadID);
    };
    switch (event.type) {
      case "message":
      case "message_reply":
